...
def run_patched_checker(monkeypatch: pytest.MonkeyPatch):
    """
    Возвращает патченный класс DiffOnlyChecker,
    ...
    def make_checker(code: str, fake_errors: list[tuple[int, int, str, type]]):
        tree = ast.parse(code)
        checker = DiffOnlyChecker(tree, filename="test_file.py")
...
def test_error_on_changed_line(run_patched_checker):
    code = "x = 1\ny=2\nz = 3"
    fake_errors = [(2, 0, "T100 Fake error", DiffOnlyChecker)]
...
def test_no_error_on_unchanged_line(run_patched_checker):
    code = "x = 1\ny = 2\nz = 3"
    fake_errors = [(1, 0, "T100 Fake error", DiffOnlyChecker)]
    results = run_patched_checker(code, fake_errors)
...
def test_multiple_errors_some_filtered(run_patched_checker):
    code = "x=1\ny = 2\nz = 3"
    fake_errors = [
        (1, 0, "T101 E1", DiffOnlyChecker),
        (2, 0, "T102 E2", DiffOnlyChecker),
        (3, 0, "T103 E3", DiffOnlyChecker),
    ]
    results = run_patched_checker(code, fake_errors)
